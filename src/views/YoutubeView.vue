...
export default {
  ...
  setup() {
    const youtubes = ref([]);
    const search = ref("garam");

    const SearchYoutubes = () => {
      fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${search.value}&key=AIzaSyCiFhVoDTSh6qtaySYS0Ji5U7BgB3Cdf_s&maxResults=28`
      )
